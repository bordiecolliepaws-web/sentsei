const langSelect = document.getElementById('lang');
        ...
        function applyRomanization() {
            ...
            if (label) label.textContent = showRomanization ? 'On' : 'Off';
        }
        ...
        function renderProgressStats() {
            ...
            if (srsDeckEl) srsDeckEl.textContent = String(srsDeck.length);
            if (srsDueEl) srsDueEl.textContent = String(getDueItems().length);
            if (srsMasteredEl) srsMasteredEl.textContent = String(srsDeck.filter(i => i.interval > 30 * DAY_MS).length);
        }
        ...
        const SRS_KEY = 'sentsei-srs';
        const DAY_MS = 86400000;

        function loadSRSDeck() {
            try {
        ...
        function saveSRSDeck() {
            localStorage.setItem(SRS_KEY, JSON.stringify(srsDeck));
        }

        function addToSRS(sentence, translation, lang, pronunciation) {
            if (!sentence || !translation) return;
            const exists = srsDeck.some(item => item.sentence === sentence && item.lang === lang);
            if (exists) return;
            srsDeck.push({
                sentence,
                translation,
        ...
        function getDueItems() {
            const now = Date.now();
            return srsDeck.filter(item => item.nextReview <= now);
        }
        ...
        function loadReviewQuestion() {
            ...
            // Build choices from SRS deck (same language)
            const sameLang = srsDeck.filter(item => item.lang === question.lang);
            const showTranslation = Math.random() > 0.5;
            ...
            if (showTranslation) {
                ...
                choices.push({ text: question.sentence, correct: true });
                choices.sort(() => Math.random() - 0.5);
                currentReview = { item: question, correctAnswer: question.sentence, choices };
            } else {
                ...
                choices.push({ text: question.translation, correct: true });
                choices.sort(() => Math.random() - 0.5);
                currentReview = { item: question, correctAnswer: question.translation, choices };
            }

            sourceEl.textContent = `Interval: ${formatTimeUntil(question.interval)}`;

            // Render choice buttons
            currentReview.choices.forEach(choice => {
                const btn = document.createElement('button');
                btn.type = 'button';
                btn.textContent = choice.text;
                btn.style.cssText = 'padding:0.75rem 1rem;border-radius:var(--radius);border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:0.95rem;cursor:pointer;text-align:left;transition:all 0.2s;';
                btn.addEventListener('click', () => {
                    ...
                    if (choice.correct) {
                        ...
                        btn.style.color = 'var(--easy)';
                        reviewCorrect++;
                        updateSRSItem(currentReview.item, true);
                    } else {
                        btn.style.background = '#4a1a1a';
                        btn.style.borderColor = 'var(--hard)';
                        btn.style.color = 'var(--hard)';
                        choicesEl.querySelectorAll('button').forEach(b => {
                            if (b.textContent === currentReview.correctAnswer) {
                                b.style.background = '#1a4a2a';
                                b.style.borderColor = 'var(--easy)';
                                b.style.color = 'var(--easy)';
                            }
                        });
                        updateSRSItem(currentReview.item, false);
                    }
                    reviewTotal++;
                    document.getElementById('review-score').textContent = `${reviewCorrect}/${reviewTotal}`;
        ...
        let learnGeneration = 0; // Incremented on each new learn request; stale async ops check this
        ...
        function startLoadingTimer() {
            stopLoadingTimer(); // Clear any existing timer first
        ...
        async function learn() {
            ...
            const sentence = sentenceInput.value.trim();
            if (!sentence) return;
            learnGeneration++; // Invalidate any in-flight segment/grammar requests
        ...
        function renderResult(data, original, reqCtx) {
            ...
            const myGeneration = learnGeneration; // Capture current generation
            if (segmentChips) {
                (async () => {
                    try {
                        ...
                        if (myGeneration !== learnGeneration) return; // Stale — new input arrived
            ...
            // Lazy-load full grammar notes on demand
            ...
            if (showGrammarBtn) {
                showGrammarBtn.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        if (myGeneration !== learnGeneration) return; // Stale — new input arrived, discard grammar
                        const bd = await bdResp.json();

                        // Merge LLM breakdown data
                        ...
                        // Update word chips with LLM data (has meanings, notes, difficulty)
                        const DIFF_LABELS = { easy: 'Easy', medium: 'Medium', hard: 'Hard' };
